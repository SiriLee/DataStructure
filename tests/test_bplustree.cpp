...
template<int m>
void insert_all(dsc::BPlusTree<m>& tree, const std::vector<int>& keys) {
    for (int k : keys) {
...
template<int m>
void verify_search(const dsc::BPlusTree<m>& tree, const std::vector<int>& present,
...
void test_leaf_split_m3() {
    // m=3: max 2 keys per leaf. Inserting 3 keys triggers leaf split.
    dsc::BPlusTree<3> tree;
...
int main() {
    ...
    std::cout << "test_bplustree: all tests passed!" << std::endl;
    return 0;
}
